...
def content_based_filtering(product_id):
    ...
    category_matched = cur.execute(
        "SELECT * FROM products WHERE category=%s", (data_cat,)
    )  # get all shirt category
...
def index():
    ...
    return render_template(
        "home.html", tshirt=tshirt, wallet=wallet, belt=belt, shoes=shoes, form=form
    )
...
class RegisterForm(Form):
    ...
    mobile = StringField(
        "", [validators.length(min=11, max=15)], render_kw={"placeholder": "Mobile"}
    )
        ...
        if (
            ...
            if picture.lower().endswith((".png", ".jpg", ".jpeg")):
                ...
                if save_photo:
                    ...
                    curs.execute(
                        "INSERT INTO product_level(product_id)" "VALUES(%s)",
                        [product_id],
                    )
                    if category == "tshirt":
...
def developer():
    ...
    if request.method == "POST" and form.validate():
        ...
        if result > 0:
            ...
            return render_template(
                "developer.html", form=form, x=x, execution_time=seconds
            )
        else:
